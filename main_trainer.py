...
from model import AntiModelIinitial, PAC
...
class Trainer():
    ...
    def evaluate(self, dataloader, criterion):
        ...
        with torch.no_grad():
            for antibody_set, antigen_set, label, cdr_infos in dataloader:
                # 构造 cdr_mask
                B = antibody_set[0].size(0)
                L_ab = antibody_set[0].size(1)
                cdr_mask = torch.zeros(B, L_ab, dtype=torch.float32, device=device)
                for j, cdr_info in enumerate(cdr_infos):
                    for region, (start, end) in cdr_info.items():
                        cdr_mask[j, start:end] = 1.0
                probs = self.model(antibody_set, antigen_set, pad_token_id=0)
    ...
    def train(self, criterion, epochs=250):
        ...
        early_stopping = EarlyStopping(patience=self.args.patience, mode=mode)
        ...
        for epoch in range(epochs):
            ...
            scheduler_metric = val_loss if self.args.monitor_metric == 'loss' else \
                            val_f1 if self.args.monitor_metric == 'f1' else val_pr_auc
            scheduler.step(scheduler_metric)

            if early_stopping(scheduler_metric):
            ...
            if self.args.monitor_metric == 'loss':
                ...
                current_metric = scheduler_metric
            ...
            if improved:
                ...
                ckpt_path = f"./ckpts/{self.args.model_name}_best.pth"
                os.makedirs("./ckpts", exist_ok=True)
                torch.save(self.model.state_dict(), ckpt_path)
                print(f"Epoch {epoch + 1}: Saved new best model ({monitor_key}={scheduler_metric:.4f})")
        ...
        print(f"\nTraining completed. Best Val Metrics: {self.best_val_metrics}")
...
def custom_collate_fn(batch):
    """
    自定义 collate 函数，确保 cdr_info 不被 default_collate 破坏
    batch 是一个 list，每个元素是 __getitem__ 的返回值:
    ([ab, at_type, ab_struct], [ag, ag_struct], label, cdr_info)
    """
    # 解包 batch
    antibody_sets = [item[0] for item in batch]
    antigen_sets = [item[1] for item in batch]
    labels = [item[2] for item in batch]
    cdr_infos = [item[3] for item in batch]  # ← 直接收集为 list，不处理

    # 对前三个使用 default_collate（它们包含 tensors）
    from torch.utils.data.dataloader import default_collate
    antibody_batch = default_collate(antibody_sets)
    antigen_batch = default_collate(antigen_sets)
    label_batch = default_collate(labels)

    return antibody_batch, antigen_batch, label_batch, cdr_infos
...
if __name__ == "__main__":
    ...
    parser.add_argument('--disable_compatibility', action='store_true',
                        help='Disable physical property guided attention for ablation study')
    ...
    fold_log_dir = os.path.join(log_dir, "single_run")
    os.makedirs(fold_log_dir, exist_ok=True)
    backup_source_files(source_files_to_backup, fold_log_dir)

    # ========== 按 Antigen Sequence 划分（8:2，抗原级别）==========
    #unique_antigens = df['Antigen Sequence'].unique()
    unique_antigens = df['vh'].unique()
    np.random.seed(args.seed)  # 确保可复现
    np.random.shuffle(unique_antigens)

    # 2:8 split
    split_idx = int(len(unique_antigens) * 0.8)
    train_antigens = set(unique_antigens[:split_idx])
    val_antigens = set(unique_antigens[split_idx:])

    #train_idx = df[df['Antigen Sequence'].isin(train_antigens)].index.tolist() # antigen split
    #val_idx = df[df['Antigen Sequence'].isin(val_antigens)].index.tolist()
    train_idx = df[df['vh'].isin(train_antigens)].index.tolist()
    val_idx = df[df['vh'].isin(val_antigens)].index.tolist()
    print(f"Total unique antigens: {len(unique_antigens)}")
    print(f"Train antigens: {len(train_antigens)} → {len(train_idx)} samples")
    print(f"Val antigens: {len(val_antigens)} → {len(val_idx)} samples")

    subset_df = df.iloc[train_idx + val_idx].copy().reset_index(drop=True)
    temp_data_path = os.path.join(fold_log_dir, "subset_for_processing.csv")
    subset_df.to_csv(temp_data_path, index=False)
    print(f"Saved subset for processing to: {temp_data_path}")

    temp_data_path = os.path.join(fold_log_dir, "subset_for_processing.csv")
    subset_df.to_csv(temp_data_path, index=False)
    print(f"Saved subset for processing to: {temp_data_path}")

    print("=== Processing data using process(...) ===")
    metadata_path, processed_dir = process(
        fold=0,
        data_path=temp_data_path,
        output_suffix="_single_run"
    )

    config_path = os.path.join(fold_log_dir, "config.json")
    with open(config_path, "w") as f:
        ...
        fold_args.update({
            ...
            'train_pos_ratio': float(df.iloc[train_idx]['ANT_Binding'].mean()),
            'val_pos_ratio': float(df.iloc[val_idx]['ANT_Binding'].mean()),
        })
    ...
    log_file = os.path.join(fold_log_dir,
                            f"{args.model_name}_{args.batch_size}_{args.epochs}_{args.latent_dim}_{args.lr}.csv")
    ...
    # ========== 保存最终结果 ==========
    with open(os.path.join(fold_log_dir, "final_results.txt"), "w") as f:
        f.write("Single Run Training Results\n")
    ...
    print(f"\n Training finished. Logs & models saved in: {fold_log_dir}")
